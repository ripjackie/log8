...
rom1[0, :3] = [ RAW_FE | HL_LDL, RAW_FF | HL_LDH, HL_EN | PC_LD | SP_DEC | CTRL_SINT | CTRL_SRST ]
rom1[1, :6] = [ PC_EN | HL_LD, HL_ENL | SP_EN | SP_DEC | ADR_EN, HL_ENH | SP_EN | SP_DEC | ADR_EN, RAW_FC | HL_LDL, RAW_FD | HL_LDH, HL_EN | PC_LD | CTRL_SRST ]

rom2 = np.zeros((64, 8), dtype=np.uint64)

NEXT = [ PC_EN | PC_INC | CTRL_SINR ]

rom2[:, :len(NEXT)] = NEXT
rom2[:, len(NEXT):len(NEXT)+1] = CTRL_SRST

step = rom2[:, len(NEXT):]
step[0x10, :1] = [ PC_EN | PC_INC | REGA_LD | CTRL_SRST ]
